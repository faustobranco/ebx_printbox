...
class pyBox:   
    def __init__(self, x, y, x2, y2, border=False, clear_screen=False):   
        ...
        self.lst_printedLines = collections.deque(maxlen=self.x2 - self.x + 1 if self.border==False else (self.x2 - self.x) - 1)

    def get_limits(self):
        rows, columns = os.popen('stty size', 'r').read().split()
        #self.max_rows = int(rows)
        #self.max_columns = int(columns)
        pyBox.__dict__['get_limits'].max_rows = rows
        pyBox.__dict__['get_limits'].max_columns = columns
    ...
    def create_box(self):
        ...
        if self.x <= 0 or self.x2 <= 0 or self.y <= 0 or self.y2 <= 0:
           return 'Err: Box size must be in screen limits: (' + str(obj_Limits.max_rows) + ' rows x ' + str(obj_Limits.max_columns) + ' columns).'
        if self.x2 < self.x:
        ...
        if self.x > obj_Limits.max_rows:
           return 'Err: Start line must by lower than screen limits: (' + str(obj_Limits.max_rows) + ' rows x ' + str(obj_Limits.max_columns) + ' columns).'
        if self.x2 > obj_Limits.max_rows:
           return 'Err: Finish line must by lower than screen limits: (' + str(obj_Limits.max_rows) + ' rows x ' + str(obj_Limits.max_columns) + ' columns).'
        if self.y > obj_Limits.max_columns:
           return 'Err: Start Column must by lower than screen limits: (' + str(obj_Limits.max_rows) + ' rows x ' + str(obj_Limits.max_columns) + ' columns).' 
        if self.y2 > obj_Limits.max_columns:
           return 'Err: Finish Column must by lower than screen limits: (' + str(obj_Limits.max_rows) + ' rows x ' + str(obj_Limits.max_columns) + ' columns).'
    ...
    def box_print(self, txt_String, new_Line=True):
        ...
        for int_Linha in range(len(self.lst_printedLines)):
            sys.stdout.write("\x1b7\x1b[%d;%df%s" % (self.x + int_Linha if self.border==False else (self.x + int_Linha) + 1, self.y if self.border==False else (self.y) + 1, (self.lst_printedLines[int_Linha][:self.y2 - self.y]) if self.border==False else (self.lst_printedLines[int_Linha][:(self.y2 - self.y)- 2])))
            sys.stdout.flush()
